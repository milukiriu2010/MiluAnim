...
class MyMathUtil {
    companion object {
        ...
        // ベクトルの大きさを求める
        // ------------------------------------------------
        fun getSize(v: FloatArray): Float {
            var st = 0f
            v.forEach { s ->
                st += s*s
            }
        ...
        // 内積を用いてベクトルのなす角を求める
        // ------------------------------------------------
        fun getAngle(va: FloatArray, vb: FloatArray): Float {
            if ( va.size != vb.size ) {
                throw java.lang.RuntimeException("vector size is different")
            }

            // ベクトルAとベクトルBを掛け、スカラー値を得る
            var sab = 0f
            va.forEachIndexed { id, a ->
                val b = vb[id]
                sab += a*b
            }
        ...
        // acos(度)
        // --------------------------------------------
        fun acosf(d: Float): Float {
            return acos(d)*180f/PI.toFloat()
        }
        ...
        // atan(度)
        // --------------------------------------------
        fun atanf(d: Float): Float {
            return atan(d)*180f/PI.toFloat()
        }
